...
data <- read.csv('C:/Users/swettstein/Desktop/data_ww.csv') %>%  
          filter( !is.na(year)) %>%  group_by %>%  
            mutate( ann_prod = as.numeric( ann_prod), 
                    ta_price = as.numeric(ta_price),
                    country  = as.factor(country),
                    xmp_gdp = ifelse( is.na(xmp_gdp), 0, xmp_gdp), # ?? changer
...
# Scatterpolot 
  pairs.panels(data[,c('country','ann_prod','coc','rol','ta_price')], 
...
# data2 <- data
# data2$Production_annuelle <-  data2$ann_prod

# Create the plots
pairs(data[,c('country','ann_prod','coc','rol','ta_price')], 
...
# Corruption
  data %>% 
    ...
    geom_line(aes(color=country ))+
    labs(color="Legend") + 
    ggtitle( 'Titre de ton graphique: Corruption') + 
    labs(y = "Corruption", x="Temps") +
    theme(plot.title = element_text(color = "black", size = 12, hjust = 0.5), 
          panel.background = element_rect(fill = "white"),
          panel.grid.major = element_line(linetype = "dashed", colour = "#F0F0F0"), 
          axis.text = element_text(colour = "black"),
...
# scatter plot entre deux variables
  ...
  data %>%  
    ...
    ggtitle( 'Titre de ton graphique') + 
...
# Regression lineaire 
  lm1 <- lm(log(ann_prod)  ~  coc*country + ta_price  , data = data) 
...
# presentation of the results for report
  ...
  stargazer(lm1,lm1,lm1,
            ...
            dep.var.caption  = "<b> Nome de la variabel dépendente </b>")

# copie le résultat dans la console (code html)
# ouvre un nouveau fichier R markdown 
# efface tout le code sauf l'entete 
# colle le résultat html
# lance le code (Ctrl + Shift + K )
# copie colle le résultat dans ton raport 
...
#            
#            coef= list(
#              fitr.WHOREP[,"Estimate"],
#              fitr.TPST[,"Estimate"],
#              fitr.FOOD[,"Estimate"],
#              fitr.INFCOM[,"Estimate"],
#              fitr.FIN[,"Estimate"],
#              fitr.REA[,"Estimate"]
#            ), 
#            se =  list( 
